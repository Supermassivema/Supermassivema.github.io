...
## 업로드 주기 (중요)
- **주 2회 · 월요일 · 금요일** (시드니 기준 오전 10시 이전 커밋 → 11시 예약이 변환/PR)
- **회차마다 정확히 3건**: `region: global` 1 + `region: korea` 1 + `region: australia` 1
- 각 지역의 그 주 **가장 중요한 뉴스 1건** 선정.

클로드 코워크는 **뉴스 1건당 마크다운 파일 1개**를 이 폴더(`_incoming/news/`)에 넣는다.
**완성본만** 둔다(임시 파일·`_to_delete` 등 금지 — 담당이 오해 없이 바로 처리하게).
